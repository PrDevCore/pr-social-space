...
import {
  Send, Calendar, Sparkles, Image as ImageIcon, Check, AlertTriangle,
  Clock, FileText, X, Plus, Eye, Loader2, Share2
} from 'lucide-react';
import { PlatformId } from '@/lib/socialspace-types';
...
export const Composer: React.FC<ComposerProps> = ({
  onPublishNow, onSchedulePost, onSaveDraft, onGenerateAiContent
}) => {
  ...
  const strictestMaxChars = Math.min(...selectedPlatforms.map((p) => PLATFORMS[p]?.maxChars || 280));
  ...
  const handleRemoveMedia = (url: string) => setMediaUrls(mediaUrls.filter((u) => u !== url));

  const handlePublish = async () => {
    if (!caption.trim() || selectedPlatforms.length === 0) return;
    setIsSubmitting(true); setSubmitMode('publish');
    try { await onPublishNow({ caption, platforms: selectedPlatforms, mediaUrls }); setCaption(''); }
    finally { setIsSubmitting(false); setSubmitMode(null); }
  };

  const handleSchedule = async () => {
    if (!caption.trim() || selectedPlatforms.length === 0 || !scheduledAt) return;
    setIsSubmitting(true); setSubmitMode('schedule');
    try {
      await onSchedulePost({ caption, platforms: selectedPlatforms, mediaUrls, scheduledAt: new Date(scheduledAt).toISOString() });
      setCaption(''); setIsScheduling(false);
    } finally { setIsSubmitting(false); setSubmitMode(null); }
  };

  const handleDraft = async () => {
    if (!caption.trim()) return;
    setIsSubmitting(true); setSubmitMode('draft');
    try { await onSaveDraft({ caption, platforms: selectedPlatforms, mediaUrls }); }
    finally { setIsSubmitting(false); setSubmitMode(null); }
  };
  ...
  const handleGenerateAi = async () => {
    ...
    try {
      ...
      if (generated) { setCaption(generated); setShowAiModal(false); }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      <div className="p-4 sm:p-6 border-b border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => setShowAiModal(true)} className="flex items-center gap-2 text-xs font-semibold px-3.5 py-1.5 rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition shadow-sm">
      ...
      <div className="p-4 sm:p-6 space-y-6">
        <div>
          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2.5">Select Publishing Channels ({selectedPlatforms.length} active)</label>
            ...
            {(Object.keys(PLATFORMS) as PlatformId[]).map((pId) => {
              ...
              return (
                <button key={pId} type="button" onClick={() => togglePlatform(pId)}
                  className={`flex items-center gap-2 p-2.5 rounded-lg border text-left text-xs font-medium transition ${isSelected ? 'bg-blue-50 border-blue-300 text-blue-900 shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300 hover:text-slate-900'}`}>
                  <div className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${isSelected ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className={`text-xs font-mono font-medium ${isOverCharLimit ? 'text-rose-600 font-bold' : 'text-slate-500'}`}>
          ...
          <div className="relative">
            <textarea rows={5} value={caption} onChange={(e) => setCaption(e.target.value)}
              ...
              className={`w-full bg-white border rounded-lg p-3.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 transition ${isOverCharLimit ? 'border-rose-400 focus:ring-rose-500' : 'border-slate-200 focus:ring-blue-500 focus:border-transparent'}`} />
            {isOverCharLimit && (
        ...
        <div>
          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Attached Media URLs</label>
        ...
        <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => setIsScheduling(!isScheduling)}
              className={`text-xs px-3 py-1 rounded-lg font-medium transition ${isScheduling ? 'bg-purple-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100'}`}>
          ...
          {isScheduling && (
            <div className="pt-2 border-t border-slate-200 flex flex-col sm:flex-row items-center gap-3">
              <div className="w-full sm:w-auto">
                <label className="block text-[11px] font-medium text-slate-500 mb-1">Target Publication Date & Time</label>
        ...
        <div className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
            ...
            <div className="flex items-center gap-1 overflow-x-auto">
              {selectedPlatforms.map((pId) => (
                <button key={pId} type="button" onClick={() => setActivePreviewTab(pId)}
                  className={`text-[11px] px-2.5 py-1 rounded-md capitalize font-medium transition ${activePreviewTab === pId ? 'bg-white text-blue-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
          ...
          <div className="p-3.5 bg-white rounded-lg border border-slate-200 text-xs space-y-2.5 max-w-xl mx-auto shadow-sm">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-700">ME</div>
              <div>
                <p className="font-bold text-slate-900 text-xs">Your Brand Account</p>
                <p className="text-[10px] text-slate-500 font-mono">{PLATFORMS[activePreviewTab]?.handleExample || '@brand'} • Just now</p>
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-200">
          <button type="button" onClick={handleDraft} disabled={isSubmitting || !caption.trim()}
            ...
            {isSubmitting && submitMode === 'draft' ? <Loader2 className="w-4 h-4 animate-spin text-slate-500" /> : <FileText className="w-4 h-4 text-slate-500" />}
            <span>Save Draft</span>
          </button>
          <div className="w-full sm:w-auto flex items-center gap-2.5">
            {isScheduling ? (
              <button type="button" onClick={handleSchedule} disabled={isSubmitting || !caption.trim() || isOverCharLimit}
                className="w-full sm:w-auto flex items-center justify-center gap-2 text-xs font-bold px-6 py-2.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white shadow-md shadow-purple-200 transition disabled:opacity-50">
                {isSubmitting && submitMode === 'schedule' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Clock className="w-4 h-4" />}
                <span>Schedule Publication</span>
              </button>
            ) : (
              <button type="button" onClick={handlePublish} disabled={isSubmitting || !caption.trim() || isOverCharLimit}
                className="w-full sm:w-auto flex items-center justify-center gap-2 text-xs font-bold px-6 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-200 transition disabled:opacity-50">
                {isSubmitting && submitMode === 'publish' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
      ...
      {showAiModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-200 rounded-xl max-w-md w-full p-6 text-slate-800 shadow-2xl relative">
            <button onClick={() => setShowAiModal(false)} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-800 hover:bg-slate-100 transition"><X className="w-5 h-5" /></button>
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 rounded-lg bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center"><Sparkles className="w-5 h-5" /></div>
              <div><h3 className="text-base font-bold text-slate-900">Gemini Social AI Assistant</h3><p className="text-xs text-slate-500">Generate platform-optimized social posts</p></div>
            </div>
            <div className="space-y-4 text-xs">
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button onClick={() => setShowAiModal(false)} className="px-4 py-2 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition">Cancel</button>
                <button onClick={handleGenerateAi} disabled={isAiLoading || !aiPrompt.trim()}
