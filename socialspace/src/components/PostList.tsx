...
export const PostList: React.FC<PostListProps> = ({
  ...
  const getStatusBadge = (status: Post['status']) => {
    switch (status) {
      ...
      case 'scheduled':
        return (
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200">
            <Clock className="w-3.5 h-3.5 text-purple-600" /> Scheduled
          </span>
        );
      case 'failed':
        return (
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
            <FileText className="w-3.5 h-3.5 text-slate-500" /> Draft
          </span>
        );
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Manage live posts, queued schedules, and API execution states
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-200 overflow-x-auto">
          {[
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-md font-medium transition shrink-0 ${
                statusFilter === tab.id
      ...
      {filteredPosts.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredPosts.map((post) => (
            <div
              key={post.id}
              className="p-4 sm:p-5 rounded-xl bg-slate-50/60 border border-slate-200 hover:border-slate-300 transition flex flex-col md:flex-row gap-4 justify-between"
            >
              {/* Main Content */}
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    {post.platforms.map((pId) => {
                      ...
                      return (
                        <span
                          key={pId}
                          className="text-[10px] font-semibold px-2 py-0.5 rounded-md border bg-white text-slate-700 border-slate-200 shadow-2xs"
                        >
                          {platform?.name || pId}
                        </span>
                      );
                ...
                {post.mediaUrls && post.mediaUrls.length > 0 && (
                  <div className="flex gap-2 pt-1">
                    {post.mediaUrls.map((url, i) => (
                      <div key={i} className="w-16 h-14 rounded-lg overflow-hidden border border-slate-200 bg-slate-100">
                ...
                <div className="flex flex-wrap items-center gap-4 text-[11px] text-slate-500 font-mono">
                  ...
                  {post.scheduledAt && (
                    <span className="text-purple-600 font-semibold flex items-center gap-1">
                      <Calendar className="w-3 h-3" /> Scheduled for: {new Date(post.scheduledAt).toLocaleString()}
                    </span>
                  )}
                  {post.error && (
                    <span className="text-rose-600 font-medium">Error: {post.error}</span>
                  )}
                </div>
              </div>

              {/* ACTION BUTTONS WIRED DIRECTLY TO POST FOR ME ENDPOINTS */}
              <div className="flex md:flex-col items-center justify-end gap-2 shrink-0 border-t md:border-t-0 md:border-l border-slate-200 pt-3 md:pt-0 md:pl-4">
                {/* Inspect Raw API Log Button */}
                <button
                  onClick={() => onInspectPayload(post)}
                  className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 transition"
                  title="Inspect raw Post For Me API response body & status"
                >
                  <Terminal className="w-3.5 h-3.5 text-blue-600" />
                  <span>Inspect API</span>
                </button>

                {/* Retry Button (if failed or draft) */}
                ...
                <button
                  ...
                  title="Delete post or cancel scheduled publication"
                >
