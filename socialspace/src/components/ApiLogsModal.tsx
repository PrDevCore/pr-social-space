        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full font-mono bg-blue-100 text-blue-800 border border-blue-200">
                  {logs.length} Executed Calls
                </span>
              </h3>
              <p className="text-xs text-slate-500">
                Live inspection of HTTP requests, payload bodies, headers, and status codes sent to publishing endpoints
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Refresh API execution logs"
            >
          ...
          {filteredLogs.length === 0 ? (
            <div className="py-20 text-center text-slate-500 font-sans">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Publish or schedule a post to see HTTP payloads dispatched to publishing endpoints.
              </p>
            </div>
          ) : (
            filteredLogs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="bg-slate-50 rounded-lg border border-slate-200 overflow-hidden shadow-xs"
                >
                  {/* Log Header */}
                  <div className="p-3 bg-white border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {log.isSimulated ? 'Sandbox Sim' : 'Live Endpoint'}
                      </span>
                    </div>
                  </div>

                  {/* Log Payloads (Request & Response) */}
                  <div className="p-3.5 grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between text-slate-500 text-[10px] uppercase font-bold">
                        ...
                        <button
                          onClick={() => copyJson(reqJson, `${log.id}_req`)}
                          className="hover:text-slate-800 flex items-center gap-1"
                        >
                          {copiedId === `${log.id}_req` ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                          <span>Copy</span>
                        </button>
                      </div>
                      <pre className="p-2.5 rounded-md bg-white border border-slate-200 text-slate-800 overflow-x-auto max-h-40 leading-relaxed">
                        {reqJson}
                      </pre>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between text-slate-500 text-[10px] uppercase font-bold">
                        ...
                        <button
                          onClick={() => copyJson(resJson, `${log.id}_res`)}
                          className="hover:text-slate-800 flex items-center gap-1"
                        >
                          {copiedId === `${log.id}_res` ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                          <span>Copy</span>
                        </button>
                      </div>
                      <pre className="p-2.5 rounded-md bg-white border border-slate-200 text-slate-800 overflow-x-auto max-h-40 leading-relaxed">
