...
import { RefreshCw, CheckCircle2, ShieldAlert, Share2, Users, Radio, ExternalLink } from 'lucide-react';
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4 sm:p-6 shadow-sm space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Authorized social channels ready for automated cross-platform posting
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
        {(Object.keys(PLATFORMS) as PlatformId[]).map((pId) => {
          ...
          const isConnected = account ? account.status === 'connected' : true;

          return (
            <div
              key={pId}
              className="p-4 rounded-xl bg-slate-50/60 border border-slate-200 hover:border-slate-300 transition flex flex-col justify-between space-y-3"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div
                    className="w-9 h-9 rounded-lg flex items-center justify-center font-bold text-xs bg-white text-slate-800 border border-slate-200 shadow-2xs"
                  >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-mono text-slate-500">
                      {account?.handle || platform.handleExample}
                    </p>
                  </div>
                </div>
                ...
                {isConnected ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">
                    <ShieldAlert className="w-3 h-3 text-amber-600" /> Re-auth
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200">
                <div className="flex items-center gap-1.5 text-slate-500">
                  ...
                  <span className="font-mono text-slate-800 font-semibold">
                    {account ? account.followerCount.toLocaleString() : '12,400'}
                  </span>
                  <span>Audience</span>
                </div>
                ...
                >
                  <span>Channel API Settings</span>
